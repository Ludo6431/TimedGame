    ...
    while(1){
		...
		switch(choix[0]){
                     ...
                     case '3' : reprise_partie_sauvegarde();
                                break;
                     case '4' : interruption(); //retour menu avec sauvegarde
                                break;
                     case '5' : pause();
                                break;
                     case '6' : reprendre(); //reprise apres pause
                                break;
                     case '7' : afficher_historique();
                                break;
                     case '8' : quitter_avec_sauvegarde(); //retour au menu avec sauvegarde
                                break;
                     case '9' : if(MenuState==M_MAIN){
					quitter();
				}
				else {
					retour_menu();
